...
(function () {
  ...
  if (reducedMotion) {
    ...
    if (hasGsap) {
      ...
      heroTl.to(heroWords.map(function (w) { return w.querySelector('span'); }), { y: 0, rotate: 0, skewY: 0, duration: wordTiming.duration, stagger: wordTiming.stagger, ease: wordTiming.ease }, 0.25)
    ...
    if (hasScrollTrigger) {
      ...
      belowFoldWords.forEach(function (el) {
        window.ScrollTrigger.create({
          ...
          onEnter: function () {
            markRevealed(el);
            window.gsap.to(el.querySelector('span'), { y: 0, rotate: 0, skewY: 0, duration: wordTiming.duration, ease: wordTiming.ease });
          },
        });
      });
